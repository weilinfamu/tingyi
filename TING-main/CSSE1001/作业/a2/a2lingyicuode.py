...
class Player:
    def __init__(self, start_strength: int, moves_remaining: int)->None:   #initialize an instance of Player
        ...
        self.moves = moves_remaining
          # is that false? just to check in gradescope.
        """ 
...
class SokobanModel:
    def __init__(self, maze_file:str)->None:
        ...
        self.potion_effect.items() = {'strength': 0, 'moves': 0}
    ...
    def attempt_move(self, direction: str) -> bool:
        ...
        if direction not in DIRECTION_DELTAS:   #other directions that are invalid
            return False
        ...
        if isinstance(Crate_new_pos, Crate):
            ...
            if isinstance(self.maze[new_row + row_offset][new_col + col_offset], Goal):
                ...
                # player's position
        """ 
        while get potion, you need delete it from the maze"""
        # potions
    ...
    def undo(self)->None:
        """
        Reverses the effects of the last valid move.
        """
        
        self.player_last_position =  self.player_position
        if not self.player_last_position:   # if it is invalid 
            self.player_last_position = self.player_last_moves  # use the last input which recorded in the last move
            return
        #giving the reversing positions for entities including crate and potion
        reverse_dic = {'w': 's', 's': 'w', 'a': 'd', 'd': 'a'}
        reverse_direction = reverse_dic[self.last_direction]  # get the reverse direction
        new_row_offset, new_col_offset = DIRECTION_DELTAS[reverse_direction]
        last_row,last_col = self.player_last_position[0] + new_row_offset, self.player_last_position[1] + new_col_offset # the reversed position
        self.player_reverse_position = (last_row, last_col)    # the reversed position
        #the last valid position to get a potion
        (new_row, new_col) = self.player_position 
        """ 
        if player took a potion, it will be reversed,and before reversed, 
        try to make a reversed potion effect
        """
        for attribute,value in self.potion_effect.items():
            if attribute == 'strength':
                self.player.add_strength(-value)
            elif attribute == 'moves':
                self.player.add_moves_remaining(-value)
        if isinstance(self.maze[new_row][new_col], Potion):
            potion = self.entities.get((new_row, new_col))
            #revert potion effect
            
            
            
            
        self.player_last_moves = self.player_last_position      #store the self.player_last_position into self.last_moves
